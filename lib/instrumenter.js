...
(function (isNode) {
	...
	function defaultWalker(node, walker) {
		...
		if (node.walking) {
			throw new Error(
				"Infinite regress: Custom walkers may NOT call walker.apply(node)",
			);
		}
		node.walking = true;
		...
		if (isArray(walkerFn)) {
			for (
				...
				if (ret.type !== type && !isLast) {
					throw new Error(
						"Only the last walker is allowed to change the node type: [type was: " +
							type +
							" ]",
					);
				}
	...
	function Instrumenter(options) {
		...
		if (this.opts.esModules && !this.opts.noAutoWrap) {
			...
			if (this.opts.debug) {
				console.log(
					"Setting noAutoWrap to true as required by esModules",
				);
			}
		}
	...
	Instrumenter.prototype = {
		...
		instrument: function (
			code,
			filename,
			afterCoverageInjectCode,
			callback,
		) {
			this.afterCoverageInjectCode = afterCoverageInjectCode;
		...
		getPreamble: function (sourceCode, emitUseStrict) {
			...
			var afterCoverageInjectCode = this.afterCoverageInjectCode || "";
			code = [
				afterCoverageInjectCode,
				"%STRICT%",
		...
		coverStatement: function (node, walker) {
			var sName, incrStatementCount, parent, grandParent;
			...
			if (node.type === SYNTAX.FunctionDeclaration.name) {
				...
				if (
					parent &&
					parent.node.type === SYNTAX.ExportNamedDeclaration.name
				) {
					return;
				}
		...
		branchName: function (type, startLine, pathLocations) {
			...
			for (i = 0; i < pathLocations.length; i += 1) {
				pathLocations[i].skip =
					pathLocations[i].skip || ignoring || undefined;
		...
		ifBranchInjector: function (node, walker) {
			var alreadyIgnoring = !!this.currentState.ignoring,
				...
				child;
			thenBody.unshift(
				astgen.statement(this.branchIncrementExprAst(bName, 0)),
			);
			elseBody.unshift(
				astgen.statement(this.branchIncrementExprAst(bName, 1)),
			);
			if (ignoreThen) {
		...
		switchBranchInjector: function (node, walker) {
			...
			for (i = 0; i < cases.length; i += 1) {
				...
				cases[i].consequent.unshift(
					astgen.statement(this.branchIncrementExprAst(bName, i)),
				);
			}
		},
		...
		conditionalBranchInjector: function (node, walker) {
			...
			node.consequent.preprocessor = this.maybeAddSkip(
				this.branchLocationFor(bName, 0),
			);
			node.alternate.preprocessor = this.maybeAddSkip(
				this.branchLocationFor(bName, 1),
			);
		...
		maybeAddSkip: function (branchLocation) {
			return function (node) {
				var alreadyIgnoring = !!this.currentState.ignoring,
					hint = this.currentState.currentHint,
					ignoreThis =
						!alreadyIgnoring && hint && hint.type === "next";
				if (ignoreThis) {
					this.startIgnore();
					node.postprocessor = this.endIgnore;
				}
		...
		logicalExpressionBranchInjector: function (node, walker) {
			...
			for (i = 0; i < leaves.length; i += 1) {
				...
				tuple.node.preprocessor = this.maybeAddSkip(
					this.branchLocationFor(bName, i),
				);
			}
		},
